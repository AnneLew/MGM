...
function getDaylength(day; latitude::Float64 = 47.8)
    latitude > 90.0 ||
        latitude < -90.0 && return error("latitude must be between 90.0 & -90.0 Degree")
...
function getTemperature(
    day;
    yearlength::Int64 = 365,
    tempDev::Float64 = 1.0,
    maxTemp::Float64 = 18.8,
    minTemp::Float64 = 1.1,
    tempDelay::Int64 = 23,
)
    return (
...
function getSurfaceIrradianceDay(
    day;
    yearlength::Int64 = 365,
    maxI::Float64 = 868.0,
    minI::Float64 = 96.0,
...
function getWaterlevel(
    ...
    minW::Float64 = -0.3,
    wDelay::Int64 = 40,
    levelCorrection::Float64 = 0.0,
)
    return (
        levelCorrection +
        (maxW - (maxW - minW) / 2 * (1 + cos((2 * pi / yearlength) * (day - wDelay))))
    ) #[m]
end

#plot(getWaterlevel, 1, 365)

function reduceNutrientConcentration(
    Biomass;
    maxNutrient::Float64 = 0.5,
    hNutrReduction::Float64 = 200.0,
)
...
function getSurfaceIrradianceHour(
    day,
    hour;
    yearlength::Int64 = 365,
    latitude::Float64 = 47.8,
    maxI::Float64 = 868.0,
    minI::Float64 = 96.0,
    iDelay::Int64 = -10,
) #times in hour after sunset
    irradianceD = getSurfaceIrradianceDay(
        day,
...
function getLightAttenuation(
    day;
    kdDev::Float64 = 1.0,
    maxKd::Float64 = 2.3,
    minKd::Float64 = 2.0,
    yearlength::Int = 365,
    kdDelay::Float64 = -10.0,
)
...
function distPlantTopFromSurface(
    day,
    height;
    LevelOfGrid::Float64 = -1.0,
    yearlength::Int64 = 365,
...
function getReducedLightAttenuation(
    day,
    Biomass;
    yearlength::Int64 = 365,
    kdDev::Float64 = 1.0,
    ...
    minKd::Float64 = 2.0,
    kdDelay::Float64 = -10.0,
    backgrKd::Float64 = 1.0,
    hTurbReduction::Float64 = 40.0,
    pTurbReduction::Float64 = 1.0,
)

    lightAttenuCoef = getLightAttenuation(
        day,
        kdDev = kdDev,
        maxKd = maxKd,
        minKd = minKd,
    ...
    return (
        lightAttenuCoefAdjusted =
            backgrKd +
            (lightAttenuCoef - backgrKd) * (hTurbReduction^pTurbReduction) /
            (Biomass^pTurbReduction + hTurbReduction^pTurbReduction)
    ) #[m^-1]
end
...
function getEffectiveIrradianceHour(
    ...
    Biomass::Float64 = 0.0,
    height::Float64 = 1.0,
    parFactor::Float64 = 0.5,
    fracReflected::Float64 = 0.1,
    iDev::Float64 = 0.0,
    plantK::Float64 = 0.02,
    fracPeriphyton::Float64 = 0.2,
    latitude::Float64 = 47.8,
    maxI::Float64 = 868.0,
    minI::Float64 = 96.0,
    iDelay::Int64 = -10,
    ...
    irrSurfHr = getSurfaceIrradianceHour(
        day,
        hour,
        yearlength = yearlength,
        latitude = latitude,
    ...
    lightWater =
        irrSubSurfHr * exp(1)^(-lightAttenuCoef * distWaterSurface - plantK * higherbiomass) # LAMBERT BEER # ÂµE/m^2*s # MÃ¶glichkeit im Exponenten: (absorptivity*c_H2O_pure*dist_water_surface))
...
function getPhotosynthesis(
    ...
    minTemp::Float64 = 1.1,
    tempDelay::Int64 = 23,
    sPhotoTemp::Float64 = 1.35,
    pPhotoTemp::Float64 = 3.0,
    hPhotoTemp::Float64 = 14.0,
    ...
    distFromPlantTop =
        distWaterSurf - distPlantTopFromSurface(
            day,
            height,
            LevelOfGrid = LevelOfGrid,
            yearlength = yearlength,
    ...
    tempFactor =
        (sPhotoTemp * (temp^pPhotoTemp)) / ((temp^pPhotoTemp) + (hPhotoTemp^pPhotoTemp)) #Â°C
...
function getNumberOfSeedsProducedByOnePlant(
    Biomass;
    seedFraction::Float64 = 0.13,
    seedBiomass::Float64 = 0.00002,
)
